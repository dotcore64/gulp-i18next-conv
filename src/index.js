...
function getConverter(file, gettextFormat) {
  switch (
    extname(file.path) // file.extname doesn't work in older vinyl used by gulp 3
  ) {
    case ".po":
    case ".pot":
    case ".mo": {
      ...
      switch (gettextFormat) {
        ...
        default: {
          throw new PluginError(
            PLUGIN_NAME,
            "Cannot determine which which file to convert to.",
          );
        }
      }
    }
        ...
        .then((contents) => {
          ...
          try {
            ...
            return Promise.reject(
              new PluginError(PLUGIN_NAME, "determineLocale failed", {
                showStack: true,
              }),
            );
          }
